...
class LeggedRobot(BaseTask):
    ...
    def _post_physics_step_callback(self):
        ...
        # gait phase update
        self.phase += self.dt
        self.phase = self.phase % self.gait_period
    ...
    def _init_buffers(self):
        ...
        # 历史接触窗口，用于 duty_factor_avg
        N = 20
        self.contact_history = torch.zeros(
            self.num_envs, len(self.feet_indices), N, device=self.device
        )

        # 步态相位（这里用 self.gait_period，在 _parse_cfg 中设置）
    ...
    def _create_envs(self):
        ...
        # 记录各条腿的 index，并定义步态相位 phi（依据书里的对角步态 / 行走步态）
        self.fl_idx = next(i for i, n in enumerate(feet_names) if "FL" in n)
        self.fr_idx = next(i for i, n in enumerate(feet_names) if "FR" in n)
        self.rl_idx = next(i for i, n in enumerate(feet_names) if "RL" in n)
        self.rr_idx = next(i for i, n in enumerate(feet_names) if "RR" in n)

        self.phi = torch.zeros(len(feet_names), device=self.device)
        # 下面这个顺序你可以调整：这里只是一个典型对角 walk 的相位示例
        self.phi[self.fl_idx] = 0.0   # 前左
        self.phi[self.fr_idx] = 0.25  # 前右
        self.phi[self.rl_idx] = 0.75  # 后左
        self.phi[self.rr_idx] = 0.5   # 后右

        # 行走步态触地系数 r > 0.5，这里取 0.7
        self.r_des = 0.7
    ...
    def _get_env_origins(self):
        if self.cfg.terrain.mesh_type in ["heightfield", "trimesh"]:
            ...
            if hasattr(self.terrain, "goals"):
                self.terrain_goals = torch.from_numpy(self.terrain.goals).to(
                    self.device
                ).to(torch.float)
                ...
                self.env_goals[:] = torch.cat(
                    (temp, last_col.repeat(1, self.cfg.env.num_future_goal_obs, 1)),
                    dim=1,
                )[:]
    ...
    def _gather_cur_goals(self, future=0):
        cur_goal_idx = self.cur_goal_idx[:, None, None] + future
        cur_goal_idx_3d = cur_goal_idx.expand(-1, -1, self.env_goals.shape[-1])
    ...
    def _parse_cfg(self, cfg):
        ...
        # 步态周期（秒）：这里定义一个默认值，可以在 cfg 里再暴露为参数
        self.gait_period = 0.8
    ...
    def _reward_duty_factor(self):
        """瞬时触地系数 r，鼓励有 3~4 条腿在地上（r≈0.7）"""
        contact = (self.contact_forces[:, self.feet_indices, 2] > 1.0).float()
        support_ratio = contact.mean(dim=1)  # ∈[0,1]
        moving = (torch.norm(self.commands[:, :2], dim=1) > 0.1).float()
        r_des = self.r_des
        err = support_ratio - r_des
        return torch.exp(-(err * err) / 0.01) * moving

    def _reward_duty_factor_avg(self):
        """窗口平均触地系数 r"""
        r_i = self.contact_history.mean(dim=2)   # 每条腿 r
        r_mean = r_i.mean(dim=1)                 # 平均 r
        r_des = self.r_des
        err = r_mean - r_des
        return torch.exp(-(err * err) / 0.01)

    def _desired_contact_pattern(self):
        """根据步态周期和相位 phi 生成理想触地模式"""
        s = (self.phase / self.gait_period).unsqueeze(1)  # [N,1]
        phi = self.phi.unsqueeze(0)                       # [1,F]
        rel = (s - phi) % 1.0
        desired_contact = (rel < self.r_des).float()
        return desired_contact

    def _reward_gait_phase(self):
        """惩罚实际触地模式偏离理想步态模式
        注意：cfg 里 gait_phase 的权重要设为负数
        """
        desired = self._desired_contact_pattern()
        actual = (self.contact_forces[:, self.feet_indices, 2] > 1.0).float()
        diff = torch.abs(actual - desired)
        return diff.mean(dim=1)

    def _reward_lateral_symmetry(self):
        """惩罚左右两侧支撑不均衡，防止一侧拖着走"""
        contact = (self.contact_forces[:, self.feet_indices, 2] > 1.0).float()
        left_support = contact[:, [self.fl_idx, self.rl_idx]].sum(dim=1)
        right_support = contact[:, [self.fr_idx, self.rr_idx]].sum(dim=1)
