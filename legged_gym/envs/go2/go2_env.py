...
class LeggedRobotR2(BaseTask):
    ...
    def _init_buffers(self):
        ...
        self.randomized_lag_tensor = torch.randint(0, self.cfg.domain_rand.lag_timesteps, (self.num_envs, 1),
        ...
        self.force_sensor_tensor = gymtorch.wrap_tensor(force_sensor_tensor).view(self.num_envs, 4,
                                                                                  6)  # for feet only, see create_env()
        ...
        self.action_history_buf = torch.zeros(self.num_envs, self.cfg.env.history_len, self.num_dofs,
                                              device=self.device, dtype=torch.float)
    ...
    def compute_observations(self):
        ...
        if self.cfg.terrain.measure_heights:
            ...
            heights = torch.clip(self.root_states[:, 2].unsqueeze(1) - 0.4 - self.measured_heights, -1,
        ...
        if control_type == "P":
            if hasattr(self.cfg.domain_rand, 'randomize_kpkd') and self.cfg.domain_rand.randomize_kpkd:
                torques = self.kp_factor * self.p_gains * (
                            joint_pos_target - self.dof_pos) - self.kd_factor * self.d_gains * self.dof_vel
            else:
    ...
    def reset_idx(self, env_ids):
        ...
        for key in self.cost_episode_sums.keys():
            self.extras["episode"]['cost_' + key] = torch.mean(
                self.cost_episode_sums[key][env_ids]) / self.max_episode_length_s
    ...
    def _get_base_heights(self, env_ids=None):
        ...
        if env_ids:
            points = quat_apply_yaw(self.base_quat[env_ids].repeat(1, self.num_base_height_points),
                                    self.base_height_points[env_ids]) + (self.root_states[env_ids, :3]).unsqueeze(1)
        else:
            points = quat_apply_yaw(self.base_quat.repeat(1, self.num_base_height_points), self.base_height_points) + (
            self.root_states[:, :3]).unsqueeze(1)
    ...
    def _cost_collision(self):
        ...
        return torch.sum(1. * (torch.norm(self.contact_forces[:, self.penalised_contact_indices, :], dim=-1) > 1.),
                         dim=1)
    ...
    def _cost_feet_contact_forces(self):
        ...
        return torch.sum(
            (torch.norm(self.contact_forces[:, self.feet_indices, :], dim=2) - self.cfg.rewards.max_contact_force).clip(
                min=0.), dim=1)
